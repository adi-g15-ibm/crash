...
bpf_init(struct bpf_info *bpf)
{
	...
	case UNINITIALIZED:
		...
		if (!VALID_STRUCT(bpf_prog) || 
		    ...
		    INVALID_MEMBER_LAZY(bpf_prog_aux) ||
		    INVALID_MEMBER_LAZY(bpf_prog_type) ||
		    INVALID_MEMBER_LAZY(bpf_prog_tag) ||
		    INVALID_MEMBER_LAZY(bpf_prog_jited_len) ||
		    INVALID_MEMBER_LAZY(bpf_prog_bpf_func) ||
		    INVALID_MEMBER_LAZY(bpf_prog_len) ||
		    INVALID_MEMBER_LAZY(bpf_prog_insnsi) ||
		    INVALID_MEMBER_LAZY(bpf_map_map_flags) ||
		    INVALID_MEMBER_LAZY(bpf_map_map_type) ||
		    INVALID_MEMBER_LAZY(bpf_prog_aux_used_maps) ||
		    INVALID_MEMBER_LAZY(bpf_prog_aux_used_map_cnt)) {
			bpf->status = FALSE;
			command_not_supported();
		}	
		/*
		...
		if (INVALID_MEMBER_LAZY(idr_idr_rt)) {
...
do_bpf(ulong flags, ulong prog_id, ulong map_id, int radix)
{
	...
	for (i = entries = 0; i < bpf->progs; i++) {
		...
		if (flags & (PROG_ID|PROG_VERBOSE)) {
			...
			len *= SIZE(bpf_insn);
			if (VALID_MEMBER_LAZY(bpf_prog_pages)) {
			...
			if (VALID_MEMBER_LAZY(bpf_prog_aux_load_time)) {
			...
			fprintf(fp, "  NAME: ");
			if (VALID_MEMBER_LAZY(bpf_prog_aux_name)) {
			...
			fprintf(fp, "  UID: ");
			if (VALID_MEMBER_LAZY(bpf_prog_aux_user) && VALID_MEMBER_LAZY(user_struct_uid)) {
...
do_map_only:
	...
	for (i = entries = 0; i < bpf->maps; i++) {
		...
		if (flags & (MAP_ID|MAP_VERBOSE)) {
			...
			fprintf(fp, "     KEY_SIZE: ");
			if (VALID_MEMBER_LAZY(bpf_map_key_size)) {
			...
			if (VALID_MEMBER_LAZY(bpf_map_value_size)) {
			...
			if (VALID_MEMBER_LAZY(bpf_map_max_entries)) {
			...
			fprintf(fp, "  MEMLOCK: ");
			if (VALID_MEMBER_LAZY(bpf_map_memory) && VALID_MEMBER_LAZY(bpf_map_memory_pages)) {
				map_pages = UINT(bpf->bpf_map_buf + LAZY_OFFSET(bpf_map_memory)
						+ LAZY_OFFSET(bpf_map_memory_pages));
				fprintf(fp, "%d\n", map_pages * PAGESIZE());
			} else if (VALID_MEMBER_LAZY(bpf_map_pages)) {
			...
			fprintf(fp, "     NAME: ");
			if (VALID_MEMBER_LAZY(bpf_map_name)) {
			...
			fprintf(fp, "  UID: ");
			if (VALID_MEMBER_LAZY(bpf_map_memory) && VALID_MEMBER_LAZY(bpf_map_memory_user))
				user = ULONG(bpf->bpf_map_buf + LAZY_OFFSET(bpf_map_memory)
						+ LAZY_OFFSET(bpf_map_memory_user));
			else if (VALID_MEMBER_LAZY(bpf_map_user))
				user = ULONG(bpf->bpf_map_buf + LAZY_OFFSET(bpf_map_user));
			else
				user = 0;

			if (user && VALID_MEMBER_LAZY(user_struct_uid)) {
				if (readmem(user + LAZY_OFFSET(user_struct_uid), KVADDR, &uid, sizeof(uint), 
				    "user_struct.uid", QUIET|RETURN_ON_ERROR))
					fprintf(fp, "%d\n", uid);
				else

...
{ 
	...
	if (DIRECT_OFFSET_UNCHECKED(qstr_len) == INVALID_OFFSET)
	...
	if (DIRECT_OFFSET_UNCHECKED(namespace_root) >= 0) {
		...
		ASSIGN_LAZY_OFFSET(task_struct_namespace) = LAZY_OFFSET(task_struct_files) +
			sizeof(void *);
...
get_pathname(ulong dentry, char *pathname, int length, int full, ulong vfsmnt)
{
	...
	do {
		...
		d_name_len = INT(dentry_buf +
			LAZY_OFFSET(dentry_d_name) + LAZY_OFFSET(qstr_len));

...
gather_in_memory_unwind_tables(void)
{
	...
	if (INVALID_SIZE(unwind_table) ||
	    LAZY_OFFSET(unwind_table_core) == INVALID_OFFSET ||
	    LAZY_OFFSET(unwind_table_init) == INVALID_OFFSET ||
	    LAZY_OFFSET(unwind_table_address) == INVALID_OFFSET ||
	    LAZY_OFFSET(unwind_table_size) == INVALID_OFFSET ||
	    LAZY_OFFSET(unwind_table_link) == INVALID_OFFSET ||
	    LAZY_OFFSET(unwind_table_name) == INVALID_OFFSET) {
		if (CRASHDEBUG(1)) 
			error(NOTE, 

...
ipcs_init(void)
{
	...
	if (VALID_MEMBER_LAZY(idr_layer_ary) && 
	...
	if (VALID_MEMBER_LAZY(idr_idr_rt)) {
...
dump_shared_memory(int specified, ulong specified_value, int verbose, ulong task)
{
	...
	if (VALID_MEMBER_LAZY(kern_ipc_perm_id)) {
...
ipc_search_array(ulong ipc_ids_p, int specified, ulong specified_value, int (*fn)(ulong, int, ulong, int, int), int verbose)
{
	...
	if (VALID_MEMBER_LAZY(ipc_id_ary_p))
...
idr_find(ulong idp, int id)
{
	...
	if (VALID_MEMBER_LAZY(idr_layer_layer)) {
...
get_shm_info(struct shm_info *shm_info, ulong shp, int id)
{
	...
	if (VALID_MEMBER_LAZY(shmid_kernel_id))
...
get_sem_info(struct sem_info *sem_info, ulong shp, int id)
{
	...
	if (VALID_MEMBER_LAZY(sem_array_sem_id))
		sem_info->semid = INT(buf + LAZY_OFFSET(sem_array_sem_id));
	else if (VALID_MEMBER_LAZY(kern_ipc_perm_id))
...
get_msg_info(struct msg_info *msg_info, ulong shp, int id)
{
	...
	if (VALID_MEMBER_LAZY(msg_queue_q_id))

...
static unsigned int __sbitmap_weight(const struct sbitmap_context *sc, bool set)
{
	...
	for (i = 0; i < sc->map_nr; i++) {
		...
		if (set) {
			...
			if (VALID_MEMBER_LAZY(sbitmap_word_cleared))
...
{
	if (VALID_MEMBER_LAZY(sbitmap_word_cleared)) /* 5.0 and later */
		...
		const struct sbitmap_context *sc)
{
	...
	if (VALID_MEMBER_LAZY(sbitmap_queue_alloc_hint))
		alloc_hint_addr = sqc->alloc_hint;
	else if (VALID_MEMBER_LAZY(sbitmap_alloc_hint)) /* 5.13 and later */
	...
	if (VALID_MEMBER_LAZY(sbitmap_queue_ws_active)) /* 5.0 and later */
	...
	if (VALID_MEMBER_LAZY(wait_queue_head_head)) /* 4.13 and later */
	...
	if (VALID_MEMBER_LAZY(sbitmap_queue_round_robin))
		fprintf(fp, "round_robin = %d\n", sqc->round_robin);
	else if (VALID_MEMBER_LAZY(sbitmap_round_robin)) /* 5.13 and later */
		fprintf(fp, "round_robin = %d\n", sc->round_robin);

	if (VALID_MEMBER_LAZY(sbitmap_queue_min_shallow_depth)) /* 4.18 and later */
...
static void sbitmap_queue_context_load(ulong addr, struct sbitmap_queue_context *sqc)
{
	...
	if (VALID_MEMBER_LAZY(sbitmap_queue_ws_active))
	...
	if (VALID_MEMBER_LAZY(sbitmap_queue_min_shallow_depth))
...
void sbitmap_context_load(ulong addr, struct sbitmap_context *sc)
{
	...
	if (VALID_MEMBER_LAZY(sbitmap_alloc_hint))
		sc->alloc_hint = ULONG(sbitmap_buf + LAZY_OFFSET(sbitmap_alloc_hint));
	if (VALID_MEMBER_LAZY(sbitmap_round_robin))
